...
export class DefDescuentosBecaService {
	...
	create(payload: Partial<DefDescuentoBeca>): Observable<{ success: boolean; data: DefDescuentoBeca; message?: string }> {
		return this.http.post<any>(this.apiUrl, payload).pipe(
			map((res: any) => ({ success: !!res?.success, data: this.normalize(res?.data), message: res?.message }))
		);
	}

	update(id: number, payload: Partial<DefDescuentoBeca>): Observable<{ success: boolean; data: DefDescuentoBeca; message?: string }> {
