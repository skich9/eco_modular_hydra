...
export class ItemsModalComponent implements OnInit, OnChanges {
	...
	get showBancarioBlock(): boolean {
		...
		// QR no muestra bloque bancario en UI
	...
	addAndClose(): void {
		if (!this.form.valid) {
			...
			this.submitError = missing.length ? `Complete los siguientes campos: ${missing.join(', ')}.` : 'Complete los campos obligatorios.';
			return;
		}
