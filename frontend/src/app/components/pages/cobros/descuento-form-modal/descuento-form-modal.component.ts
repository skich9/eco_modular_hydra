...
export class DescuentoFormModalComponent implements OnInit {
	...
	@Output() guardar = new EventEmitter<{ cod_ceta: string; nombre: string; gestion: string; pensum: string; turno: string }>();
	...
	open(defaults?: Partial<{ cod_ceta: string; nombre: string; gestion: string; pensum: string; turno: string }>): void {
		...
		if (this.hasEstudiante) {
			// Habilitar form global, pero bloquear campos críticos
