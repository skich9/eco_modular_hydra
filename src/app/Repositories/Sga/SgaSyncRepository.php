...
class SgaSyncRepository
{
	...
	private function loadLocalInscripcionesBySource(array $sourceIds, string $carreraLabel, ?string $gestion = null): array
	{
		...
		$grouped = [];
		$query = DB::table('inscripciones')
			->select('source_cod_inscrip', 'cod_inscrip', 'cod_pensum', 'tipo_inscripcion', 'cod_curso')
			->where('carrera', $carreraLabel)
		...
		foreach ($rows as $row) {
			$grouped[(int) $row->source_cod_inscrip][] = [
				...
				'cod_curso' => isset($row->cod_curso) ? trim((string) $row->cod_curso) : '',
			];
		}
	...
	/**
	 * Sincroniza descuentos asignados en SGA hacia descuentos y descuento_detalle.
	 *
	 * Reglas de mapeo:
	 * - Semestrales: kardex_economico.descuento_convenio -> descuento_detalle.monto_descuento
	 *   aplicado a todas las cuotas de la inscripción local.
	 * - Por cuota: descuento_parcial + descuento_parcial_detalle (nro/num_cuota) -> cuota específica.
	 *
	 * @param string $source sga_elec|sga_mec|all
	 * @param int $chunk Tamaño de lote
	 * @param bool $dryRun Si true, solo cuenta sin persistir
	 * @return array Resumen de sincronización
	 */
	public function syncDescuentosSga(string $source, int $chunk = 1000, bool $dryRun = false, ?string $gestion = null): array
	{
		...
		$summary = [
			...
			'detalles_inserted' => 0,
			'detalles_updated' => 0,
			'semestrales_applied' => 0,
			'parciales_applied' => 0,
			'skipped_missing_inscripcion' => 0,
			'skipped_missing_cuota' => 0,
			'skipped_conflict' => 0,
			'overrides_applied' => 0,
			'skipped_invalid' => 0,
		];

		if (!Schema::hasTable('descuentos') || !Schema::hasTable('descuento_detalle') || !Schema::hasTable('asignacion_costos')) {
			$summary['error'] = 'Faltan tablas locales requeridas: descuentos, descuento_detalle o asignacion_costos';
			return $summary;
		}
		if (!Schema::hasTable('def_descuentos_beca')) {
			$summary['error'] = 'Falta tabla def_descuentos_beca para resolver cod_beca';
			return $summary;
		}

		$defaultUserId = (int) (env('HYDRA_DEFAULT_USER_ID', env('SYNC_DEFAULT_USER_ID', 1)));
		$defaultCodBecaEnv = (int) env('SGA_SYNC_DESCUENTOS_DEFAULT_COD_BECA', 0);
		$becasRows = DB::table('def_descuentos_beca')->get(['cod_beca', 'nombre_beca', 'beca']);
		$becaNameMap = [];
		foreach ($becasRows as $bRow) {
			$key = $this->normalizeLookupKey((string) ($bRow->nombre_beca ?? ''));
			if ($key !== '' && !isset($becaNameMap[$key])) {
				$becaNameMap[$key] = (int) ($bRow->cod_beca ?? 0);
			}
		}

		// Cargar IDs de descuentos institucionales desde parametros_economicos
		// y agregar aliases de nombres SGA → cod_beca correcto
		$diParamIds = [];
		try {
			$diKeys = ['dinstitucionalmanana', 'dinstitucionaltarde', 'dinstitucionalnoche'];
			$diRows = DB::table('parametros_economicos')
				->whereIn(DB::raw('LOWER(TRIM(nombre))'), $diKeys)
				->select('nombre', 'valor')
				->get();
			foreach ($diRows as $pr) {
				$pKey = strtolower(trim((string) ($pr->nombre ?? '')));
				$pVal = (int) ($pr->valor ?? 0);
				if ($pKey !== '' && $pVal > 0) {
					$diParamIds[$pKey] = $pVal;
				}
			}
		} catch (\Throwable $e) {}

		// Alias SGA para "Descuento instituciónal Bs.-35 Tarde" → cod_beca de Tarde
		if (isset($diParamIds['dinstitucionaltarde']) && $diParamIds['dinstitucionaltarde'] > 0) {
			foreach ([
				'DESCUENTO INSTITUCIÓNAL BS.-35 TARDE',
				'DESCUENTO INSTITUCIONAL BS.-35 TARDE',
			] as $alias) {
				if (!isset($becaNameMap[$alias])) {
					$becaNameMap[$alias] = $diParamIds['dinstitucionaltarde'];
				}
			}
		}

		$defaultCodBeca = null;
		if ($defaultCodBecaEnv > 0) {
			$exists = DB::table('def_descuentos_beca')->where('cod_beca', $defaultCodBecaEnv)->value('cod_beca');
			if ($exists) {
				$defaultCodBeca = (int) $exists;
			}
		}
		if ($defaultCodBeca === null) {
			$fallback = DB::table('def_descuentos_beca')->where('beca', false)->orderBy('cod_beca')->value('cod_beca');
			if ($fallback) {
				$defaultCodBeca = (int) $fallback;
			}
		}
		if ($defaultCodBeca === null) {
			$fallbackAny = DB::table('def_descuentos_beca')->orderBy('cod_beca')->value('cod_beca');
			if ($fallbackAny) {
				$defaultCodBeca = (int) $fallbackAny;
			}
		}
		$summary['default_cod_beca'] = $defaultCodBeca;
		if ($defaultCodBeca === null) {
			$summary['error'] = 'No se pudo resolver cod_beca: configure SGA_SYNC_DESCUENTOS_DEFAULT_COD_BECA o cargue def_descuentos_beca';
			return $summary;
		}

		foreach ($connections as $conn) {
			$carreraLabel = $this->getCarreraLabel($conn);
			$sourceInscripcionesGestion = [];
			if ($gestion !== null && Schema::connection($conn)->hasTable('registro_inscripcion') && Schema::connection($conn)->hasColumn('registro_inscripcion', 'gestion')) {
				$sourceInscripcionesGestion = DB::connection($conn)
					->table('registro_inscripcion')
			...
			// 1) Descuentos semestrales: kardex_economico
			if (Schema::connection($conn)->hasTable('kardex_economico')) {
				$querySemestral = DB::connection($conn)
					->table('kardex_economico')
					->whereNotNull('descuento_convenio')
					->where('descuento_convenio', '>', 0);

				if ($gestion !== null && Schema::connection($conn)->hasColumn('kardex_economico', 'gestion')) {
					$querySemestral->where('gestion', $gestion);
				} elseif ($gestion !== null) {
					if (!empty($sourceInscripcionesGestion)) {
						$querySemestral->whereIn('cod_inscrip', $sourceInscripcionesGestion);
					} else {
						$querySemestral->whereRaw('1 = 0');
					}
				}

				$querySemestral
					->orderBy('cod_inscrip')
					->chunk($chunk, function ($rows) use (&$summary, $dryRun, $defaultUserId, $defaultCodBeca, $becaNameMap, $diParamIds, $carreraLabel, $gestion) {
						if (empty($rows)) {
							return;
						}

						$summary['total_rows'] += count($rows);
						$summary['semestrales_rows'] += count($rows);
						...
						foreach ($rows as $r) {
							$monto = (float) ($r->descuento_convenio ?? 0);
							$codCeta = (int) ($r->cod_ceta ?? 0);
							$codInsSga = (int) ($r->cod_inscrip ?? 0);
							$codPensumSga = trim((string) ($r->cod_pensum ?? ''));
							$tipoInsSga = $this->normalizeTipoInscripcion($r->kardex_economico ?? 'NORMAL');

							if ($monto <= 0 || $codCeta === 0 || $codInsSga === 0 || $codPensumSga === '') {
								$summary['skipped_invalid']++;
								continue;
							}

							if (!isset($pensumMap[$codPensumSga]) || !isset($localInscripciones[$codInsSga])) {
								$summary['skipped_missing_inscripcion']++;
								continue;
							}

							$codPensumLocal = (string) $pensumMap[$codPensumSga];
							$inscLocal = $this->pickLocalInscripcion($localInscripciones[$codInsSga], $tipoInsSga, $codPensumLocal);
							if (!$inscLocal) {
								$summary['skipped_missing_inscripcion']++;
								continue;
							}

							$codInsLocal = (int) $inscLocal['cod_inscrip'];
							$tipoInsLocal = (string) ($inscLocal['tipo_inscripcion'] ?? 'NORMAL');
							$nombre = trim((string) ($r->tipo_descuento ?? ''));
							if ($nombre === '') {
								$nombre = 'Descuento semestral SGA';
							}
							$obs = isset($r->observaciones) ? (string) $r->observaciones : null;
							$codBeca = $this->resolveCodBecaByNameAndShift(
								$nombre, $inscLocal['cod_curso'] ?? '', $diParamIds, $becaNameMap, $defaultCodBeca
							);

							$maestro = $this->resolveOrCreateDescuentoSgaMaster(
								$codCeta,
								$codPensumLocal,
								$codInsLocal,
								$defaultUserId,
							...
							$cuotas = DB::table('asignacion_costos')
								->where('cod_inscrip', $codInsLocal)
								->where('cod_pensum', $codPensumLocal)
								->orderBy('numero_cuota')
								->get(['id_asignacion_costo', 'numero_cuota']);

							if ($cuotas->isEmpty()) {
								$summary['skipped_missing_cuota']++;
								continue;
							}

							foreach ($cuotas as $c) {
								$ok = $this->upsertDescuentoDetalleCuota(
									$maestro,
									$codInsLocal,
									$codPensumLocal,
									(int) ($c->id_asignacion_costo ?? 0),
									(int) ($c->numero_cuota ?? 0),
									$monto,
									null,
									$obs,
									$tipoInsLocal,
			...
			// 2) Descuentos por cuota: descuento_parcial + descuento_parcial_detalle
			if (Schema::connection($conn)->hasTable('descuento_parcial') && Schema::connection($conn)->hasTable('descuento_parcial_detalle')) {
				$queryParcial = DB::connection($conn)
					->table('descuento_parcial as dp')
					->join('descuento_parcial_detalle as dpd', 'dpd.id_descuento_parcial', '=', 'dp.id_descuento_parcial')
					->whereNotNull('dpd.monto_descuento')
					->where('dpd.monto_descuento', '>=', 0);

				if ($gestion !== null && Schema::connection($conn)->hasColumn('descuento_parcial', 'gestion')) {
					$queryParcial->where('dp.gestion', $gestion);
				} elseif ($gestion !== null) {
					if (!empty($sourceInscripcionesGestion)) {
						$queryParcial->whereIn('dp.cod_inscrip', $sourceInscripcionesGestion);
					} else {
						$queryParcial->whereRaw('1 = 0');
					}
				}

				if (Schema::connection($conn)->hasColumn('descuento_parcial_detalle', 'valido')) {
					$queryParcial->where(function ($q) {
						$q->where('dpd.valido', true)
							->orWhere('dpd.valido', 1)
							->orWhereIn('dpd.valido', ['t', 'T', 'true', 'TRUE']);
					});
				}

				$queryParcial
					->orderBy('dp.id_descuento_parcial')
					->orderBy('dpd.id_descuento_detalle')
					->chunk($chunk, function ($rows) use (&$summary, $dryRun, $defaultUserId, $defaultCodBeca, $carreraLabel, $gestion) {
						if (empty($rows)) {
							return;
						}

						$summary['total_rows'] += count($rows);
						$summary['parciales_rows'] += count($rows);
						...
						foreach ($rows as $r) {
							$monto = (float) ($r->monto_descuento ?? 0);
							$codCeta = (int) ($r->cod_ceta ?? 0);
							$codInsSga = (int) ($r->cod_inscrip ?? 0);
							$codPensumSga = trim((string) ($r->cod_pensum ?? ''));
							$nroCuota = (int) ($r->nro_cuota ?? ($r->num_cuota ?? 0));
							$tipoInsSga = $this->normalizeTipoInscripcion($r->kardex_economico ?? 'NORMAL');
							$codArchivo = isset($r->cod_archivo) ? trim((string) $r->cod_archivo) : null;
							$obs = isset($r->observacion) ? (string) $r->observacion : (isset($r->observaciones) ? (string) $r->observaciones : null);

							if ($monto < 0 || $codCeta === 0 || $codInsSga === 0 || $nroCuota <= 0 || $codPensumSga === '') {
								$summary['skipped_invalid']++;
								continue;
							}
	...
	private function resolveOrCreateDescuentoSgaMaster(
		int $codCeta,
		string $codPensum,
		int $codInscrip,
		int $idUsuario,
		...
		$nombre = trim($nombre);
		if ($codCeta <= 0 || $codInscrip <= 0 || $codPensum === '' || $nombre === '') {
			return null;
		}

		$existing = DB::table('descuentos')
			->where('cod_ceta', $codCeta)
			->where('cod_pensum', $codPensum)
			->where('cod_inscrip', $codInscrip)
			->where('tipo', $tipo)
			->where('nombre', $nombre)
			->orderByDesc('id_descuentos')
			->first(['id_descuentos']);

		if ($existing) {
			$summary['maestros_updated']++;
			if (!$dryRun) {
				$updateData = [
				...
				if ($codBeca !== null) {
					$updateData['cod_beca'] = $codBeca;
				}
				DB::table('descuentos')
					->where('id_descuentos', (int) $existing->id_descuentos)
					->update($updateData);
			}
			return (int) $existing->id_descuentos;
		}

		$summary['maestros_inserted']++;
		if ($dryRun) {
			return -1;
		}

		return (int) DB::table('descuentos')->insertGetId([
			'cod_ceta' => $codCeta,
			'cod_pensum' => $codPensum,
	...
	private function resolveCodBecaByName(string $nombre, array $becaNameMap, ?int $defaultCodBeca): ?int
	{
		$key = $this->normalizeLookupKey($nombre);
		if ($key !== '' && isset($becaNameMap[$key])) {
			return (int) $becaNameMap[$key];
		}

		return $defaultCodBeca !== null ? (int) $defaultCodBeca : null;
	}

	/**
	 * Resuelve cod_beca considerando aliases de nombres SGA y turno del estudiante.
	 * - "Descuento Institucional 5%" → Mañana o Noche según el turno del cod_curso
	 * - "Descuento instituciónal Bs.-35 Tarde" → ya mapeado en becaNameMap; cae al lookup normal
	 */
	private function resolveCodBecaByNameAndShift(
		string $nombre,
		string $codCurso,
		array $diParamIds,
		array $becaNameMap,
		...
		// Alias SGA: "Descuento Institucional 5%" → Mañana o Noche según turno
		if ($key === 'DESCUENTO INSTITUCIONAL 5%') {
			$turno = $this->extractTurnoFromCodCurso($codCurso);
			if ($turno === 'N' && isset($diParamIds['dinstitucionalnoche']) && $diParamIds['dinstitucionalnoche'] > 0) {
				return $diParamIds['dinstitucionalnoche'];
			}
			if (isset($diParamIds['dinstitucionalmanana']) && $diParamIds['dinstitucionalmanana'] > 0) {
				return $diParamIds['dinstitucionalmanana'];
			}
		}

		return $this->resolveCodBecaByName($nombre, $becaNameMap, $defaultCodBeca);
	}

	/**
	 * Extrae el turno (M/T/N) del cod_curso usando el último carácter del último segmento.
	 * Ejemplo: "04-MTZ-101M" → 'M', "04-MTZ-102T" → 'T', "04-MTZ-103N" → 'N'
	 */
	private function extractTurnoFromCodCurso(string $codCurso): string
	{
		$raw = strtoupper(trim($codCurso));
		if ($raw === '') { return 'M'; }
		$parts = explode('-', $raw);
		$suffix = trim((string) end($parts));
		if ($suffix === '') { return 'M'; }
	...
	private function upsertDescuentoDetalleCuota(
		int $idDescuento,
		int $codInscrip,
		string $codPensum,
		?int $idCuota,
		...
		if ($idDescuento === 0 || $codInscrip <= 0 || $codPensum === '' || $monto < 0) {
			$summary['skipped_invalid']++;
			return false;
		}

		$resolvedIdCuota = $idCuota ? (int) $idCuota : 0;
		if ($resolvedIdCuota <= 0 && $numeroCuota > 0) {
			$resolvedIdCuota = (int) (DB::table('asignacion_costos')
				->where('cod_inscrip', $codInscrip)
				->where('cod_pensum', $codPensum)
				->where('numero_cuota', $numeroCuota)
				->value('id_asignacion_costo') ?? 0);
		}

		if ($resolvedIdCuota <= 0) {
			$summary['skipped_missing_cuota']++;
			return false;
		}

		$asig = DB::table('asignacion_costos')
			->where('id_asignacion_costo', $resolvedIdCuota)
			->first(['id_descuentoDetalle']);

		$currentDetailId = (int) ($asig->id_descuentoDetalle ?? 0);
		$currentDetail = null;
		if ($currentDetailId > 0) {
			$currentDetail = DB::table('descuento_detalle')
				->where('id_descuento_detalle', $currentDetailId)
				->first(['id_descuento_detalle', 'id_descuento']);
		}

		if (!$currentDetail) {
			$currentDetail = DB::table('descuento_detalle')
				->where('id_cuota', $resolvedIdCuota)
				->orderByDesc('id_descuento_detalle')
				->first(['id_descuento_detalle', 'id_descuento']);
		}

		if ($currentDetail && (int) ($currentDetail->id_descuento ?? 0) !== $idDescuento) {
			$existingIdDescuento = (int) ($currentDetail->id_descuento ?? 0);
			$existingTipo = strtoupper(trim((string) (DB::table('descuentos')
				->where('id_descuentos', $existingIdDescuento)
				->value('tipo') ?? '')));
			$incomingTipo = strtoupper(trim($tipoDescuento));

			// SGA parcial es más específico y debe reemplazar al descuento semestral por cuota.
			$allowOverride = $incomingTipo === 'SGA_PARCIAL' && $existingTipo === 'SGA_SEMESTRAL';
			if (!$allowOverride) {
		...
		if ($currentDetail) {
			$summary['detalles_updated']++;
			if (!$dryRun && $idDescuento > 0) {
				DB::table('descuento_detalle')
					->where('id_descuento_detalle', (int) $currentDetail->id_descuento_detalle)
					->update([
						'id_descuento' => $idDescuento,
						'id_inscripcion' => $codInscrip,
						'id_cuota' => $resolvedIdCuota,
						'monto_descuento' => round($monto, 2),
						'cod_Archivo' => $codArchivo,
						'observaciones' => $observaciones,
						'tipo_inscripcion' => $tipoInscripcion,
						'updated_at' => now(),
					]);

				DB::table('asignacion_costos')
					->where('id_asignacion_costo', $resolvedIdCuota)
					->update(['id_descuentoDetalle' => (int) $currentDetail->id_descuento_detalle]);
			}
			return true;
		}

		$summary['detalles_inserted']++;
		if ($dryRun || $idDescuento < 0) {
			return true;
		}

		$idDet = (int) DB::table('descuento_detalle')->insertGetId([
			'id_descuento' => $idDescuento,
			'id_inscripcion' => $codInscrip,
			'id_cuota' => $resolvedIdCuota,
			'monto_descuento' => round($monto, 2),
			'cod_Archivo' => $codArchivo,
			'observaciones' => $observaciones,
			'tipo_inscripcion' => mb_substr($tipoInscripcion, 0, 100),
		...
		DB::table('asignacion_costos')
			->where('id_asignacion_costo', $resolvedIdCuota)
			->update(['id_descuentoDetalle' => $idDet]);
	...
	/**
	 * Sincroniza cobros de MATERIAL_ADICIONAL (libros/material extra) desde SGA hacia cobro.
	 *
	 * Origen: tabla material_adicional (sga_elec|sga_mec). Como la tabla no tiene columna gestion,
	 * ésta se obtiene mediante JOIN con registro_inscripcion (ri.cod_inscrip = ma.cod_inscrip).
	 *
	 * Estrategia por fila:
	 *  - Se resuelve cod_pensum local vía mapPensumsByCarrera (soporta divergencia de códigos).
	 *  - Se busca inscripción local por (carrera, cod_ceta, cod_pensum_local, gestion, tipo_inscripcion).
	 *    Si no existe, se marca ERROR en sga_sync_cobros (queda reintentable en futuras corridas).
	 *  - cod_tipo_cobro = MATERIAL_EXTRA (validado contra tipo_cobro).
	 *  - id_item: cascada nombre_libro -> insumo -> concepto -> item genérico "material extra" -> null.
	...
	public function syncCobrosMaterialAdicional(string $source, ?string $gestion = null, int $chunk = 1000, bool $dryRun = false, ?int $codCetaFilter = null, ?string $codPensumFilter = null, bool $trace = false): array
	{
		...
		if (!Schema::hasTable('cobro') || !Schema::hasTable('sga_sync_cobros')) {
			return compact('source','gestion','total','inserted','skipped','errors','skippedSynced','skippedMissingUser','skippedMissingInscripcion','skippedMissingItem');
		}
		if (!Schema::connection($source)->hasTable('material_adicional')) {
			return compact('source','gestion','total','inserted','skipped','errors','skippedSynced','skippedMissingUser','skippedMissingInscripcion','skippedMissingItem');
		}
		if (!Schema::connection($source)->hasTable('registro_inscripcion')) {
			return compact('source','gestion','total','inserted','skipped','errors','skippedSynced','skippedMissingUser','skippedMissingInscripcion','skippedMissingItem');
		}
		...
		$codTipoCobroMaterial = $this->resolveCodTipoCobro('MATERIAL_EXTRA');

		$genericItemId = null;
		try {
			if (Schema::hasTable('items_cobro')) {
				$genericItemId = DB::table('items_cobro')
					->whereRaw('LOWER(TRIM(nombre_servicio)) IN (?, ?, ?, ?)', [
						'material extra', 'material adicional', 'libros', 'material educativo'
					])
					->value('id_item');
				$genericItemId = $genericItemId ? (int) $genericItemId : null;
			}
		} catch (\Throwable $e) { $genericItemId = null; }

		$baseQuery = DB::connection($source)
			->table('material_adicional as ma')
			->join('registro_inscripcion as ri', function ($join) {
				$join->on('ri.cod_inscrip', '=', 'ma.cod_inscrip')
					->on('ri.cod_ceta', '=', 'ma.cod_ceta')
					->on('ri.cod_pensum', '=', 'ma.cod_pensum');
			})
			->when($gestion !== null, function ($q) use ($gestion) {
				$q->where('ri.gestion', (string) $gestion);
			})
			->when($codCetaFilter !== null, function ($q) use ($codCetaFilter) {
				$q->where('ma.cod_ceta', (int) $codCetaFilter);
			})
			->when($codPensumFilter !== null && trim((string)$codPensumFilter) !== '', function ($q) use ($codPensumFilter) {
		...
		$baseQuery
			->select('ma.*', 'ri.gestion as ri_gestion', 'ri.cod_curso as ri_cod_curso')
			->orderBy('ma.cod_ceta')
			->orderBy('ma.cod_pensum')
			->orderBy('ma.cod_inscrip')
			->orderBy('ma.num_pago_mat')
			->chunk($chunk, function ($rows) use (&$total, &$inserted, &$skipped, &$errors, &$skippedSynced, &$skippedMissingUser, &$skippedMissingInscripcion, &$skippedMissingItem, $dryRun, $trace, $source, $gestion, $defaultUserId, $carreraLabel, $codTipoCobroMaterial, $genericItemId) {
				$total += count($rows);
				if (empty($rows)) { return; }
				...
				foreach ($rows as $r) {
					...
					if ($lib !== '') { $libroNames[] = $lib; }
					$ins = mb_strtolower(trim((string) ($r->insumo ?? '')));
					if ($ins !== '') { $insumoNames[] = $ins; }
					$cpt = mb_strtolower(trim((string) ($r->concepto ?? '')));
					if ($cpt !== '') { $conceptoNames[] = $cpt; }
				}
				$sourcePks = array_values(array_unique(array_filter($sourcePks)));
				$nicknames = array_values(array_unique(array_filter($nicknames)));
				$sgaPensums = array_values(array_unique($sgaPensums));

				$pensumMap = $this->mapPensumsByCarrera($sgaPensums, $carreraLabel);

				$already = [];
				if (!empty($sourcePks)) {
					$syncQuery = DB::table('sga_sync_cobros')
						->where('source_conn', $source)
						->where('source_table', 'material_adicional')
						->whereIn('source_pk', $sourcePks);
					if (!$dryRun) {
						$syncQuery->where('status', 'OK');
					}
				...
				$itemMap = [];
				if (Schema::hasTable('items_cobro')) {
					$allItemNames = array_values(array_unique(array_merge($libroNames, $insumoNames, $conceptoNames)));
					if (!empty($allItemNames)) {
						$itemRows = DB::table('items_cobro')
							->whereIn(DB::raw('LOWER(TRIM(nombre_servicio))'), $allItemNames)
							->select('id_item', 'nombre_servicio')
							->get();
						foreach ($itemRows as $it) {
							$itemMap[mb_strtolower(trim((string) $it->nombre_servicio))] = (int) $it->id_item;
						}
					}
				}
				...
				foreach ($rows as $r) {
					try {
						...
						$codCeta = (int) ($r->cod_ceta ?? 0);
						$codPensumSga = trim((string) ($r->cod_pensum ?? ''));
						if ($codCeta <= 0 || $codPensumSga === '') { $skipped++; continue; }

						$codPensumLocal = $pensumMap[$codPensumSga] ?? $codPensumSga;

						$gestionRow = trim((string) ($r->ri_gestion ?? ''));
						if ($gestion !== null && $gestionRow === '') { $gestionRow = (string) $gestion; }
						if ($gestionRow === '') {
							$skipped++;
							$this->markSyncCobro($source, 'material_adicional', $sourcePk, $codCeta, $codPensumLocal, '', $r, null, null, ($dryRun && !$trace), ($dryRun ? 'DRY_ERROR' : 'ERROR'), 'Sin gestion resuelta (registro_inscripcion no pareado)');
							continue;
						}
						...
						$idFormaCobro = $this->resolveFormaCobro((string) ($r->code_tipo_pago ?? ''));
						$numPagoMat = (int) ($r->num_pago_mat ?? 0);
						$orden = isset($r->orden) ? (int) $r->orden : $numPagoMat;
						$monto = (float) ($r->costo_total ?? 0);
						$cobroCompleto = (bool) ($r->pago_completo ?? true);
						$nroRecibo = isset($r->num_comprobante) && (int) $r->num_comprobante > 0 ? (int) $r->num_comprobante : null;
						$nroFactura = isset($r->num_factura) && (int) $r->num_factura > 0 ? (int) $r->num_factura : null;

						$razon = null;
						try {
							$razon = trim((string) ($r->razon ?? $r->razon_social ?? ''));
							if ($razon === '') { $razon = null; }
						} catch (\Throwable $e) { $razon = null; }
						$nroDocumentoPago = null;
						try {
							$nroDocumentoPago = trim((string) ($r->nro_documento_pago ?? ''));
							if ($nroDocumentoPago === '') { $nroDocumentoPago = null; }
						} catch (\Throwable $e) { $nroDocumentoPago = null; }

						$nombreLibro = trim((string) ($r->nombre_libro ?? ''));
						$insumo = trim((string) ($r->insumo ?? ''));
						$concepto = trim((string) ($r->concepto ?? ''));
						if ($concepto === '') {
							if ($nombreLibro !== '') { $concepto = $nombreLibro; }
							elseif ($insumo !== '') { $concepto = $insumo; }
							else { $concepto = 'Material Extra'; }
						}

						$idItem = null;
						$libKey = mb_strtolower($nombreLibro);
						$insKey = mb_strtolower($insumo);
						$cptKey = mb_strtolower($concepto);
						if ($libKey !== '' && isset($itemMap[$libKey])) {
							$idItem = $itemMap[$libKey];
						} elseif ($insKey !== '' && isset($itemMap[$insKey])) {
							$idItem = $itemMap[$insKey];
						} elseif ($cptKey !== '' && isset($itemMap[$cptKey])) {
							$idItem = $itemMap[$cptKey];
						} elseif ($genericItemId !== null) {
							$idItem = $genericItemId;
						} else {
							$idItem = null;
							$skippedMissingItem++;
						}
						...
						if ($dryRun) {
							$this->markSyncCobro($source, 'material_adicional', $sourcePk, $codCeta, $codPensumLocal, $gestionRow, $r, null, null, ($dryRun && !$trace), 'DRY_OK', null);
							$inserted++;
							continue;
						}
						...
						$insQ = DB::table('inscripciones')
							->where('carrera', $carreraLabel)
							->where('cod_ceta', (int) $codCeta)
							->where('cod_pensum', (string) $codPensumLocal)
							->where('gestion', (string) $gestionRow);
						if (Schema::hasColumn('inscripciones', 'tipo_inscripcion')) {
							$insQ->where('tipo_inscripcion', $tipoIns);
						}
						$insRow = $insQ->orderByDesc('cod_inscrip')->first(['cod_inscrip']);
						if (!$insRow || !isset($insRow->cod_inscrip)) {
							$skippedMissingInscripcion++;
							$this->markSyncCobro($source, 'material_adicional', $sourcePk, $codCeta, $codPensumLocal, $gestionRow, $r, null, null, ($dryRun && !$trace), 'ERROR', 'No existe inscripcion local para cod_ceta/cod_pensum/gestion/tipo_inscripcion');
							continue;
						}
						$codInsLocal = (int) $insRow->cod_inscrip;

						$maxNroCobroRetries = 25;
						$nroCobro = null;
						for ($attempt = 1; $attempt <= $maxNroCobroRetries; $attempt++) {
							$candidate = $this->nextDocCounter('COBRO:' . $anioCobro);
							$exists = DB::table('cobro')
								->where('cod_ceta', (int) $codCeta)
								->where('cod_pensum', (string) $codPensumLocal)
								->where('tipo_inscripcion', (string) $tipoIns)
								->where('nro_cobro', (int) $candidate)
								->exists();
							if (!$exists) { $nroCobro = $candidate; break; }
						}
						if ($nroCobro === null) {
							throw new \RuntimeException('No se pudo encontrar nro_cobro libre para (' . $codCeta . ', ' . $codPensumLocal . ', ' . $tipoIns . ') tras ' . $maxNroCobroRetries . ' intentos (PK cobro colisiona por contador por-anio_cobro)');
						}
						...
						DB::transaction(function () use ($source, $gestionRow, $now, $anioCobro, $nroCobro, $fechaCobro, $codCeta, $codPensumLocal, $tipoIns, $codInsLocal, $monto, $cobroCompleto, $idUsuario, $idFormaCobro, $nroFactura, $nroRecibo, $r, $sourcePk, $nrCorrelativo, $nbCorrelativo, $isEfectivo, $isBancario, $razon, $nroDocumentoPago, $orden, $codTipoCobroMaterial, $idItem, $concepto, &$inserted) {
							$cobroData = [
								'cod_ceta' => $codCeta,
								'cod_pensum' => $codPensumLocal,
								'tipo_inscripcion' => $tipoIns,
								...
								'observaciones' => isset($r->observaciones) ? (string) $r->observaciones : null,
								'concepto' => $concepto,
								'cod_tipo_cobro' => $codTipoCobroMaterial,
								'id_usuario' => $idUsuario,
								'id_forma_cobro' => $idFormaCobro,
								'tipo_documento' => null,
								'medio_doc' => null,
								'pu_mensualidad' => 0,
								'order' => max(0, (int) $orden) % 128,
								'descuento' => 0,
								'id_cuentas_bancarias' => null,
								'nro_factura' => $nroFactura,
								'nro_recibo' => $nroRecibo,
							...
							try {
								if ($nroRecibo && Schema::hasTable('recibo')) {
									...
									if (Schema::hasColumn('recibo', 'periodo_facturado')) {
										$recData['periodo_facturado'] = $gestionRow;
									}
									if (Schema::hasColumn('recibo', 'cod_tipo_doc_identidad')) {
										$recData['cod_tipo_doc_identidad'] = null;
									}
							...
							try {
								if ($nroFactura && Schema::hasTable('factura')) {
									...
									if (Schema::hasColumn('factura', 'periodo_facturado')) {
										$factData['periodo_facturado'] = $gestionRow;
									}
									if (Schema::hasColumn('factura', 'cliente')) {
										$factData['cliente'] = $razon;
									}
							...
							try {
								...
								$detalle = $concepto !== '' ? $concepto : 'Material Extra';
								$obsOriginal = isset($r->observaciones) ? (string) $r->observaciones : null;
								$fechaNota = substr((string) $fechaCobro, 0, 10);
								$anioFull = $anioCobro;
								$anio2 = (int) date('y', strtotime((string) $fechaCobro));
								...
								if ($isBancario && $nbCorrelativo && Schema::hasTable('nota_bancaria')) {
									...
									$nroDeposito = '';
									try {
										$nroDeposito = (string) ($r->nro_deposito ?? '');
										$nroDeposito = trim($nroDeposito);
									} catch (\Throwable $e) { $nroDeposito = ''; }
									$nroCuenta = '';
									try {
										$nroCuenta = (string) ($r->nro_cuenta ?? '');
										$nroCuenta = trim($nroCuenta);
									} catch (\Throwable $e) { $nroCuenta = ''; }
									$idCuenta = null;
									$bancoDest = '';
									...
									if (Schema::hasColumn('nota_bancaria', 'banco_origen')) {
										$nbData['banco_origen'] = isset($r->banco_origen) ? (string) $r->banco_origen : '';
									}
									if (Schema::hasColumn('nota_bancaria', 'nro_tarjeta')) {
										$nbData['nro_tarjeta'] = isset($r->nro_tarjeta) ? (string) $r->nro_tarjeta : null;
									}
									DB::table('nota_bancaria')->insert($nbData);
								}
							} catch (\Throwable $e) {}

							$this->markSyncCobro($source, 'material_adicional', $sourcePk, $codCeta, $codPensumLocal, $gestionRow, $r, $nroCobro, $anioCobro, false, 'OK', null);
							$inserted++;
						});
					} catch (\Throwable $e) {
						$errors++;
						...
						$sourcePk = $this->makeMaterialAdicionalSourcePk($r);
						try {
							Log::error('SGA sync cobros material: insert failed', [
								'source' => $source,
								'gestion' => $gestion ?? 'ALL',
								'cod_ceta' => $codCeta,
								'cod_pensum' => $codPensum,
						...
						if ($sourcePk !== '') {
							$this->markSyncCobro($source, 'material_adicional', $sourcePk, $codCeta, $codPensum, (string) ($gestion ?? ''), $r, null, null, ($dryRun && !$trace), ($dryRun ? 'DRY_ERROR' : 'ERROR'), $e->getMessage());
						}
					}
				}
			});
	...
	private function makeMaterialAdicionalSourcePk($r): string
	{
		$codCeta = (string) (isset($r->cod_ceta) ? $r->cod_ceta : '');
		$codPensum = (string) (isset($r->cod_pensum) ? $r->cod_pensum : '');
		$codIns = (string) (isset($r->cod_inscrip) ? $r->cod_inscrip : '');
		$kardex = (string) (isset($r->kardex_economico) ? $r->kardex_economico : '');
		$numPagoMat = (string) (isset($r->num_pago_mat) ? $r->num_pago_mat : '');
		$pk = trim($codCeta) . '|' . trim($codPensum) . '|' . trim($codIns) . '|' . trim($kardex) . '|' . trim($numPagoMat);
		return $pk === '||||' ? '' : $pk;
	}
}
