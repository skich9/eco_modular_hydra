...
class CuisRepository
{
    ...
    public function getVigenteOrCreate2(int $codigo_ambiente,int $codigo_sucursal, int $puntoVenta = 0): array
	{
		...
		// Solicitar CUIS al SIAT
		$resp = $this->codes->cuis2($codigo_ambiente,$codigo_sucursal,$puntoVenta);
		if (!isset($resp['RespuestaCuis'])) {
			throw new Exception('Respuesta CUIS inválida: ' . json_encode($resp));
		}
		$rc = $resp['RespuestaCuis'];
