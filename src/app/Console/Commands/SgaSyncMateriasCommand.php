...
class SgaSyncMateriasCommand extends Command
{
	protected $signature = 'sga:sync-materias {source=all : sga_elec|sga_mec|all} {--chunk=1000} {--dry-run}';
	protected $description = 'Sincroniza materias desde SGA hacia la tabla local materia con mapeo de pensum';
	...
	public function handle(SgaSyncRepository $repo): int
	{
		...
		foreach ($sources as $src) {
			try {
				$res = $repo->syncMaterias($src, $chunk, $dry);
				$summary[$src] = $res;
