...
class SgaSyncUsuariosCommand extends Command
{
	protected $signature = 'sga:sync-usuarios {source=all : sga_elec|sga_mec|all} {--chunk=1000} {--dry-run}';
	protected $description = 'Sincroniza usuarios (usuario) desde SGA hacia la tabla usuarios';
	...
	public function handle(SgaSyncRepository $repo): int
	{
		...
		foreach ($sources as $src) {
			try {
				$res = $repo->syncUsuarios($src, $chunk, $dry);
				$summary[$src] = $res;
