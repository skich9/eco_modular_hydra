...
class SgaSyncGestionesCommand extends Command
{
	protected $signature = 'sga:sync-gestiones {source=all : sga_elec|sga_mec|all} {--chunk=1000} {--dry-run}';
	protected $description = 'Sincroniza la tabla gestion desde SGA hacia la tabla local gestion';
	...
	public function handle(SgaSyncRepository $repo): int
	{
		...
		foreach ($sources as $src) {
			try {
				$res = $repo->syncGestiones($src, $chunk, $dry);
				$summary[$src] = $res;
