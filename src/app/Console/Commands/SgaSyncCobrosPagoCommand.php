...
class SgaSyncCobrosPagoCommand extends Command
{
	protected $signature = 'sga:sync-cobros-pago
	...
	protected $description = 'Sincroniza cobros desde SGA (pago + registro_inscripcion) hacia cobro/cobros_detalle_regular por gestion (NORMAL/ARRASTRE)';
	...
	public function handle(SgaSyncRepository $repo)
	{
		...
		foreach ($sources as $src) {
			try {
				$res = $repo->syncCobrosPagoPorGestion($src, $gestion, $chunk, $dry, $codCeta, $codPensum, $trace);
				$summary[$src] = $res;
				...
				if ($dry && !$trace) {
					$this->line("[{$src}] Nota: DRY_RUN no inserta en cobro/cobros_detalle_regular ni en sga_sync_cobros (use --trace para registrar trazabilidad).");
				}
				if ($dry && $trace) {
					$this->line("[{$src}] Nota: DRY_RUN+TRACE NO inserta en cobro/cobros_detalle_regular; solo registra trazabilidad en sga_sync_cobros.");
				}
				if (!$dry) {
					$this->line("[{$src}] Nota: WRITE inserta cobros en cobro/cobros_detalle_regular y trazabilidad en sga_sync_cobros.");
				}
				if ($skUsr > 0) {
					$this->line("[{$src}] missing_user_used_default={$skUsr}: SGA.pago.usuario no se encontró en usuarios.nickname. Se usó SYNC_DEFAULT_USER_ID (ver logs para muestras).");
				}
			} catch (\Throwable $e) {
